 ...
 */
public class UpdatePayPasswordActivity extends XCBaseActivity implements View.OnClickListener{
    ...
    private class MyTask extends AsyncTask<SettingDao,String,MemberVOData>{
        ...
        protected void onPreExecute() {
            super.onPreExecute();
            progressDialog = new MyProgressDialog(UpdatePayPasswordActivity.this);
            progressDialog.showDialog(R.string.please_wait);
        }

        @Override
        protected MemberVOData doInBackground(SettingDao... params) {
            return params[0].updateMemberPayPassword(member.getMember().getId(),member.getMember().getAccount(),oldPasswordStr,passwordStr);
        }
