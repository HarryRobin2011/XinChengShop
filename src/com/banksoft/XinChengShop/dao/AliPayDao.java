...
import com.banksoft.XinChengShop.utils.alipay.SignUtils;
...
public class AliPayDao extends BaseDao{
    ...
    public AliPayDao(Context mContext) {
        super(mContext);
    }

    public void aliPayMethod(AlipayInfo info,AlipayOrderInfo order){
        String orderInfo = getOrderInfo(info,order);
        // 对订单做RSA 签名
     ...
     *
     */
    private String getOrderInfo(AlipayInfo info,AlipayOrderInfo order) {

        // 签约合作者身份ID
        String orderInfo = "partner=" + "\"" + info.getPartner() + "\"";

        // 签约卖家支付宝账号
        orderInfo += "&seller_id=" + "\"" + info.getAliAccount() + "\"";
        ...
        // 商品金额
        orderInfo += "&total_fee=" + "\"" + order.getMoney() + "\"";

        // 服务器异步通知页面路径
        orderInfo += "&notify_url=" + "\"" + order.getNoticeUrl()
                + "\"";
